...
def main(comb_label, hz_para_list):
    ...
    for i_bootstrap in range(n_bootstrap):
        ...
        # ----------------------------------------------------------------------------
        # Loading data directly from the specified file
        dataset_df = pd.read_csv("/Users/ayushchaudhary/models_review/data_normal.csv")

        # ----------------------------------------------------------------------------

        x_data = dataset_df["score"].values.reshape(-1, 1)
        ...
        enc_age = OneHotEncoder(handle_unknown="ignore", sparse_output=False)
        ...
        # ICV_bins_train, bin_edges = pd.qcut(
        #     dataset_df["EstimatedTotalIntraCranialVol"],
        #     q=10,
        #     retbins=True,
        #     labels=bin_labels,
        # )
        ...
        # if comb_label == 1:
        #     y_data = np.concatenate((one_hot_age, one_hot_gender), axis=1).astype(
        #         "float32"
        #     )
        # elif comb_label == 2:
        #     y_data = np.concatenate((one_hot_age, one_hot_ICV_train), axis=1).astype(
        #         "float32"
        #     )
        # elif comb_label == 3:
        #     y_data = np.concatenate((one_hot_gender, one_hot_ICV_train), axis=1).astype(
        #         "float32"
        #     )
        # else:
        #     y_data = np.concatenate(
        #         (one_hot_age, one_hot_gender, one_hot_ICV_train), axis=1
        #     ).astype("float32")
        ...
        def compute_ll(x, x_recon):
            return tf.reduce_mean(
                tf.reduce_sum(x_recon.log_prob(x), 1, keepdims=True), 0
            )
        ...
        def train_step(batch_x, batch_y, pre, total):
            ...
            ae_grads = ae_tape.gradient(
                total_loss, encoder.trainable_variables + decoder.trainable_variables
            )
            ae_optimizer.apply_gradients(
                zip(ae_grads, encoder.trainable_variables + decoder.trainable_variables)
            )

            # -------------------------------------------------------------------------------------------------------------
            # Discriminator
            with tf.GradientTape() as dc_tape:
                real_distribution = tf.random.normal(
                    [batch_x.shape[0], z_dim], mean=0.0, stddev=1.0
                )
            ...
            dc_optimizer.apply_gradients(
                zip(dc_grads, discriminator.trainable_variables)
            )
        ...
        for epoch in range(n_epochs):
            ...
            for _, (batch_x, batch_y) in enumerate(train_dataset):
                ...
                ae_loss, kl_loss, total_loss, dc_loss, dc_acc, gen_loss = train_step(
                    batch_x, batch_y, pre, total
                )
                pre += batch_x.shape[0]
            ...
            print(
                "{:4d}: TIME: {:.2f} ETA: {:.2f} AE_LOSS: {:.4f} KL_LOSS: {:.4f} TOTAL_LOSS: {:.4f} DC_LOSS: {:.4f} DC_ACC: {:.4f} GEN_LOSS: {:.4f}".format(
                    epoch,
                    epoch_time,
