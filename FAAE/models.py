...
class EmbeddedLayer(keras.Layer):
    def call(self, inputs, mode=0):
        return tf.shape(inputs)[mode]
...
class Sampling(layers.Layer):
    ...
    def call(self, inputs):
        ...
        batch = EmbeddedLayer()(z_mean, 0)
        dim = EmbeddedLayer()(z_mean, 1)
...
def make_encoder_model_v1(n_features, h_dim, z_dim):
    ...
    batch = EmbeddedLayer()(z_mean, mode=0)
    dim = EmbeddedLayer()(z_mean, mode=1)
    epsilon = tf.keras.backend.random_normal(
        shape=(tf.shape(batch)[0], tf.shape(dim)[0])
    )
