...
def generate_synthetic_data(dist_type="normal", n_samples=500, seed=42, **kwargs):
    """
    ...
    # 1. Generate Age from a uniform distribution
    age_min = kwargs.get("age_min", 20)
    age_max = kwargs.get("age_max", 80)
    ages = np.random.uniform(low=age_min, high=age_max, size=n_samples)

    # 2. Generate Thickness according to dist_type
    if dist_type == "normal":
        # mean_thickness=0.5, var_thickness=0.1, for example
        mean_thickness = kwargs.get("mean_thickness", 0.5)
        var_thickness = kwargs.get("var_thickness", 0.1)
        std_thickness = np.sqrt(var_thickness)

        thickness_raw = np.random.normal(
            loc=mean_thickness, scale=std_thickness, size=n_samples
        )
        thickness = np.clip(thickness_raw, 0, 1)
        ...
        # alpha_thk=2.0, beta_thk=2.0, for example
        alpha_thk = kwargs.get("alpha_thk", 0.05)
        beta_thk = kwargs.get("beta_thk", 0.05)

        thickness_raw = np.random.beta(alpha_thk, beta_thk, size=n_samples)
        thickness = np.clip(
            thickness_raw, 0, 1
        )  # naturally in [0,1], but clipped for safety

    elif dist_type == "gmm":
        # means_thk=[0.3, 0.7], stds_thk=[0.1, 0.05], weights_thk=[0.5, 0.5]
        means_thk = kwargs.get("means_thk", [0.3, 0.7])
        ...
        for _ in range(n_samples):
            ...
            sample = np.random.normal(means_thk[comp], stds_thk[comp])
        ...
        df_thk = kwargs.get("df_thk", 3)
        loc_thk = kwargs.get("loc_thk", 0.5)
        scale_thk = kwargs.get("scale_thk", 0.2)

        thickness_raw = loc_thk + scale_thk * np.random.standard_t(
            df_thk, size=n_samples
        )
        thickness = np.clip(thickness_raw, 0, 1)
...
if __name__ == "__main__":
    ...
    dist_type = "beta"  # could be "gamma", "beta", "gmm", or "long_tailed"
    ...
    X, y = generate_synthetic_data(
        dist_type=dist_type, n_samples=500, seed=42, **params
    )
